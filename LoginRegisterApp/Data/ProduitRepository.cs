...
{
    // Read-oriented on purpose: 5.4 says this app never inserts products
    // directly, that comes from the external stock sync. The only write
    // here is stock adjustment when a DemandeAchat is validated.
    public static class ProduitRepository
    {
        // Used by ProductSyncService to know whether a ReferenceExterne already
        // exists locally (= update) or not (= brand new product, triggers a notification).
        ...
        public static Produit CreateFromSync(Produit produit)
        {
            ...
            // Check if Marque column exists in local Produit table
        ...
        // Called when a DemandeAchat is validated (5.5 - "vérifie la disponibilité
        // du stock"). Pass a negative quantite to deduct.
        ...
        // Atomic Mutex Stock Deduction: Returns true if stock was successfully deducted,
        // false if stock was insufficient or consumed by another concurrent transaction/user.
        ...
        // Deletes or disables local products that no longer exist in the external stock DB
        public static void DeleteMissingFromSync(HashSet<string> validExternalRefs)
        {
            ...
            foreach (int id in idsToDelete)
            {
                ...
                {
                    // If referenced by a purchase request, set Stock to 0 and mark name
